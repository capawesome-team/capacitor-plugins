...
This is useful if the database identifiers are no longer available,
for example after the web app has been reloaded.
...
| **`version`**           | <code>number</code>             | The version of the database schema. If provided, the plugin will check the schema version and apply migrations if necessary. If not provided, the latest version of the upgrade statements will be used, if any. If neither `version` nor `upgradeStatements` are provided, the database version will not be managed by the plugin. **Attention:** The version must be an integer between 1 and 2147483647. On Electron and Web, the call is rejected otherwise.                                                                                                                                                                                                                                                                                                                                                                                                                   |                    | 0.1.0 |
