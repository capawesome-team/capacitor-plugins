...
| **`theme`**            | <code>'auto' \| 'light' \| 'dark' \| 'auto-spinner' \| 'light-spinner' \| 'dark-spinner'</code> | Choose the theme that the datetime picker should have. With `auto` the system theme is used. This value overwrites the `theme` configuration value. Only available on Android and iOS. Spinner options only available on Android |                                             | 0.0.1 |
