...
| **`icon`**        | <code>string \| number</code> | The icon to display. On **Android**, the icon is the constant value of the `R.drawable` enum. On **iOS**, the icon can be one of the following: - The constant value of the `UIApplicationShortcutIcon.IconType` enum. - A system symbol name. - Name of the image asset from the asset catalogue. **Attention**: On **iOS**, the icon and the description must be used together. | 6.1.0 |
