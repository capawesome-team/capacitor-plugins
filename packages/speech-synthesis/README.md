...
| -------------------------- | --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- | ----- |
| **`audioSessionCategory`** | <code><a href="#audiosessioncategory">AudioSessionCategory</a></code> | The audio session category to use for speech synthesis. The audio session will be automatically activated with this category when speech starts and deactivated when speech ends, restoring the previous audio session state. With the `PLAYBACK`, `PLAY_AND_RECORD` and `MULTI_ROUTE` categories, audio from other apps (e.g. music) is ducked (volume reduced) while speaking and restored afterwards. With the default `SOLO_AMBIENT` category, audio from other apps is interrupted. Only available on iOS. | <code>AudioSessionCategory.SoloAmbient</code> | 8.0.0 |
