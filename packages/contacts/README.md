...
```
Use the `capacitor-plugins` skill from `capawesome-team/skills` to install the `@capawesome-team/capacitor-contacts` plugin in my project.
```
...
Open the native contact picker and let the user select one or more contacts. Use the `fields` option to specify which contact properties should be returned and the `multiple` option to let the user select more than one contact:
...
| **`limit`**  | <code>number</code>                                   | Limit the number of contacts returned.  | <code>20</code>                                                                                                                                                                               | 7.4.0 |
...
| Prop           | Type                                                        | Description                                                                                                                                                                                                                                                                | Default                                                                                                                                                                                                                  | Since |
| -------------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| **`fields`**   | <code>(keyof <a href="#contact">Contact</a>)[]</code>       | The fields to return for the contact. On Android 17 and newer, the contact picker only lists contacts that have at least one of the requested fields and never returns the `account` and `note` fields. Only available on Android and iOS.                                 | <code>['birthday', 'displayName', 'emailAddresses', 'familyName', 'givenName', 'id', 'jobTitle', 'middleName', 'namePrefix', 'nameSuffix', 'organizationName', 'phoneNumbers', 'postalAddresses', 'urlAddresses']</code> | 7.4.0 |
| **`multiple`** | <code>boolean</code>                                        | Whether to allow selecting multiple contacts. Ignored if the `property` option is set. On Android, only supported on Android 17 and newer, where the user can select at most 100 contacts.                                                                                 | <code>false</code>                                                                                                                                                                                                       | 7.0.0 |
